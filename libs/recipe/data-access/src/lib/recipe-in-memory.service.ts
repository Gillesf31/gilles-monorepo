...
const SCENARIOS = {
  ...
  single: [
    new Recipe(
      '1',
      'Pasta Carbonara',
      ['200g de spaghetti', '100g de guanciale', '3 jaunes d\'œuf', '50g de Pecorino Romano', 'Poivre noir'],
      [
...
export class RecipeInMemoryService extends RecipeService {
  private readonly recipes$ = new BehaviorSubject<Recipe[]>(SCENARIOS[ACTIVE_SCENARIO]);
  ...
  getRecipe(id: string): Observable<Recipe | undefined> {
    return this.recipes$.pipe(map(recipes => recipes.find(r => r.id === id)));
  }

  updateRecipe(id: string, recipe: NewRecipe): Observable<Recipe> {
    const updated = new Recipe(id, recipe.title, recipe.ingredients, recipe.instructions);
    this.recipes$.next(this.recipes$.value.map(r => r.id === id ? updated : r));
    return of(updated);
  }
